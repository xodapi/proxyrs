...
pub fn generate_csv(store: &UsageStore) -> String {
    ...
    for r in &records {
        lines.push(format!("{},{},{},{},{},{},{},{},{},{},{}", r.day, r.model, r.requests, r.ok, r.fail, r.rate_limited, r.total_tokens, r.prompt_tokens, r.completion_tokens, r.latency_ms_avg, r.cost));
    }
    lines.join("\n") + "\n"
}
...
fn collect_records(store: &UsageStore) -> Vec<ExportRecord> {
    ...
    let mut day_model_map: HashMap<(String, String), Vec<&crate::usage_store::StoredEvent>> = HashMap::new();
    for e in &events {
        if e.ts < cutoff_ts { continue; }
    ...
    let mut records: Vec<ExportRecord> = day_model_map.into_iter()
        .map(|((day, model), events)| {
            ...
            for e in &events {
                rec.requests += 1;
                if e.ok { rec.ok += 1; } else { rec.fail += 1; }
                if e.rate_limited { rec.rate_limited += 1; }
